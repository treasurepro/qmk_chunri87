...
uint8_t qbat_start = 0;
uint8_t qbat_save = 255;
uint16_t qbat_get = 100;
uint16_t qbat_get_buff[10];
uint16_t qbat_get_sum = 0;
uint16_t qbat_get_counter = 0;
uint16_t qbat_send = 255;
uint8_t qbat_counter_up = 0;
uint8_t qbat_counter_down = 0;
uint32_t mm_bat_req_time = 1000;
    ...
    if (MATRIX_SPLIT_IS_MASTER()) {
        ...
        if (!bt_init_time && (timer_elapsed32(last_time) >= (mm_bat_req_time))) {
            if ((mm_eeconfig.devs != DEVS_USB) && (bts_is_busy() == 0) && bts_info.bt_info.active) {
                ...
                bts_send_vendor(v_get_qbat);
                if(qbat_get_counter < 10)
                {
                    qbat_get_counter ++;
                }
                else if(qbat_get_counter == 10)
                {
                    qbat_get_counter = 11;
                    mm_bat_req_time = 3000;

                    qbat_get = bts_info.bt_info.qbat;
                    qbat_get = (qbat_get*100)/85;
                    if(qbat_get >100){
                        qbat_get = 100;
                    }
                    qbat_send = qbat_get;
                    bts_send_bat_pvol(qbat_send);   
                    qbat_save = qbat_send;   
                ...
                else 
                {
                    ...
                    // if(qbat_get_counter < 10){
                    //     qbat_get_buff[qbat_get_counter] = qbat_send;
                    //     qbat_get_sum += qbat_get_buff[qbat_get_counter]; 
                    //     qbat_get_counter ++;
                    // }
                    // else{
                    //     qbat_get_sum = qbat_get_sum/9;
                    //     qbat_get_counter = 1;
                    //     bts_send_bat_pvol(qbat_get_sum);               
                    // }  
                    
                    // bts_send_bat_pvol(bts_info.bt_info.qbat);
                    bts_send_bat_pvol(qbat_send);   
                    qbat_save = qbat_send;   
                }
    ...
    switch (mm_eeconfig.devs) {
        ...
        case DEVS_2G4: {
            if (reset != false) {
                ...
                bts_send_manufacturer((char *)USBSTR(MM_DONGLE_MANUFACTURER), sizeof(USBSTR(MM_DONGLE_MANUFACTURER)) - 2);
                bts_send_product((char *)USBSTR(MM_DONGLE_PRODUCT), sizeof(USBSTR(MM_DONGLE_PRODUCT)) - 2);
                bts_send_vpid(VENDOR_ID, PRODUCT_ID);
                bts_send_vendor(v_clear);
                bts_send_vendor(v_pair);
            } else {
...
void mm_set_sleep_state(mm_sleep_status_t state) {
    ...
    bts_send_vendor(v_get_qbat); // 查询一次电量信息   
